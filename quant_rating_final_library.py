# -*- coding: utf-8 -*-
"""
Created on Mon Jun  8 15:44:36 2020
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  8 15:43:41 2020
...
def count_neg_values(df):
    ...
    no_neg_values      =  df[numerical_features].lt(0).sum()
    neg_value_percent  = no_neg_values/df[numerical_features].count(0)*100
...
def default_probabilty_to_rating(default_probability, table_rating):
    ...
    for i,interval in enumerate(list_of_intervals):
        if(default_probability in interval):
...
def get_data_for_learning(df):
    ...
    """
    numerical_features = list(df.columns[df.dtypes==np.float64]) 
    for elem in ['MOODYS_ENCODED_RATING','FITCH_ENCODED_RATING','SP_ENCODED_RATING']:
    ...
    for date in list_dates[4:-4]:
        ...
        data_for_learning.loc[(slice(None),date),'DELTA_BLOOMBERG_ENCODED_RATING']    = np.clip(data_for_learning.loc[(slice(None),date),'DELTA_BLOOMBERG_ENCODED_RATING'],-1,1)
...
def encode_rating_bloom(df,feature_rating,table_rating_bloomberg):
    defaul_prob_list = list(df[feature_rating])
    rating_code = []
    for default_probability in defaul_prob_list:
        rating_code.append(default_probabilty_to_rating(default_probability, table_rating_bloomberg))
    return rating_code
