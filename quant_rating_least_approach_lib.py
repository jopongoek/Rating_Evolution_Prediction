...
def scaler_data(df):
    ...
    numerical_features.remove('BB_1YR_DEFAULT_PROB')
...
def get_data_for_learning(df):
    ...
    for elem in ['ALTMAN_Z_SCORE','BB_1YR_DEFAULT_PROB']:
    ...
    for date in list_dates[4:-4]:
        ...
        data_for_learning.loc[(slice(None),date),'Evolution_Rating']    = np.clip(-rating_encoded_ahead.values+rating_encoded.values,0,1)
...
def get_quality_scores(model,model_name, X_train, X_test, y_train, y_test):
    ...
    #train
    X_train_predict = np.clip(model.predict(X_train),-1,1)   
...
def encode_rating_bloom(df,feature_rating,table_rating_bloomberg):
    defaul_prob_list = np.abs(df[feature_rating])
    rating_code = []
...
def get_quality_scores_details_bloom(y,y_bloom_predict):
    """return a dataframe of the scoring of the model
    """
    ...
    res.iloc[0,0] = '{:.2%}'.format(sk_metrics.recall_score(y, y_bloom_predict, average=None)[0])
    res.iloc[1,0] = '{:.2%}'.format(sk_metrics.recall_score(y, y_bloom_predict, average=None)[1])
    res.iloc[2,0] = '{:.2%}'.format(sk_metrics.recall_score(y, y_bloom_predict, average=None)[2])
    res.iloc[3,0] = '{:.2%}'.format(sk_metrics.precision_score(y, y_bloom_predict, average=None)[0])
    res.iloc[4,0] = '{:.2%}'.format(sk_metrics.precision_score(y, y_bloom_predict, average=None)[1])
    res.iloc[5,0] = '{:.2%}'.format(sk_metrics.precision_score(y, y_bloom_predict, average=None)[2])
    res.iloc[6,0] = '{:.2%}'.format(sk_metrics.f1_score(y, y_bloom_predict, average=None)[0])
    res.iloc[7,0] = '{:.2%}'.format(sk_metrics.f1_score(y, y_bloom_predict, average=None)[1])
    res.iloc[8,0] = '{:.2%}'.format(sk_metrics.f1_score(y, y_bloom_predict, average=None)[2])
