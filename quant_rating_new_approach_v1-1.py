...
df_data['BLOOMBERG_RATING'] = encode_rating_bloom(df_data,'BB_1YR_DEFAULT_PROB', table_rating_bloomberg)

df_data.dropna(subset=['RATING_ENCODED','BLOOMBERG_RATING'], inplace=True)
...
for elem in ['RATING_ENCODED','BB_1YR_DEFAULT_PROB','ALTMAN_Z_SCORE','BLOOMBERG_RATING']:
...
model_rf_reg = Pipeline(steps=[('preprocessor', preprocessor),
                       ('regressor', rf_regression)
                     ])
...
print(get_quality_scores_reg(model_rf_reg,'Random_Forest_Regression', X_train, X_test,y_train, y_test))
...
y_bloom_pred = list(df_data.BLOOMBERG_RATING.astype(int))
y_true = list(df_data.RATING_ENCODED.astype(int))